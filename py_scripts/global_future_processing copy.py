...
#TODO delete this so inpput_dir and output_dir are taken from the CLI
input_dir=gsdms_dir
output_dir="/home/ubuntu/mnt/Alex/gsdms_alex/outputs/t2_out"
##################################################



def set_no_datavalue(infile_path, outfolder):
...
def get_stats_raster(infile_path, silent=True):
    ...
    # Deletes shapefile once it has calculated statistics
    os.remove(shapefile_path) 
    
    # Deletes .dbf, .prj and .shx files
    dir_name=os.path.dirname(shapefile_path)
    os.system(os.path.join(dir_name,"rm *.dbf"))
    os.system(os.path.join(dir_name,"rm *.prj"))
    os.system(os.path.join(dir_name,"rm *.shx"))
...
if __name__=="__main__":
    ...
    ssps=["ssp126"]#["ssp126","ssp245","ssp370","ssp585"]
    periods=["2021-2040"]#["2021-2040","2041-2060","2061-2080","2081-2100"]

    for ssp in ssps:
        for per in periods:
            print(f"Processing australia future bioclim variables for {ssp} and period {per}")
            input_dir=os.path.join(input_dir,f"out_{ssp}_{per}")
            output_dir=os.path.join(output_dir,f"out_aus_{ssp}_{per}")
            run_pipeline_directory(input_dir,output_dir)
