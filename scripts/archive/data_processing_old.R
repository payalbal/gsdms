...
hurtt_out <- file.path(gsdms_dir, "outputs", "hurtt")
...
## ------------------------------------------------------------------------- ##
dst_folder <- file.path(gsdms_dir, "distance_layers")
...
roadsub <- roads[(roads$FCLASS == (1:3) & roads$EXS == 1),]
...
## $$ TO RUN - >> Population density ####
## Distance is calculated from all values for pop density of 60
infile <- file.path(gsdms_dir, "population/SEDAC/gpw-v4-population-density-rev11_2020_30_sec_tif", "gpw_v4_population_density_rev11_2000_30_sec.tif")
...
## ------------------------------------------------------------------------- ##
## $$ TO FIX - Process environmental layers according to global mask ####
## ------------------------------------------------------------------------- ##
...
## Hansen treecover
tree <- list.files(file.path(gsdms_dir, "Hansen_treecover"), 
...
## Distance layers
dst_layers <- list.files(file.path(gsdms_dir, "distance_layers"),
                         full.names = TRUE)
...
## >> Combine filenames ####
cov_files <- c(srtm, soil, 
               bio_current[1:2], bio_future[1:2], 
               tree, dst_layers[1:2])
...
## NOTE: Only run step IF CRS is missing, skip otherwise
## Resamplig method used in gdal_translate: bilinear (https://support.esri.com/en/technical-article/000005606)
## NOTE: ## this step is needed to resolve the error: ERROR 1: Too many points (xx out of xx) failed to transform, unable to compute output bounds. Warning 1: Unable to compute source region for output window x,x,x,x, skipping.
infiles <- cov_files
lapply(infiles, gdalsrsinfo)
new_crs = "EPSG:4326"

outfiles <- file.path(output_dir, paste0(tools::file_path_sans_ext(basename(infiles)), "_wgs.tif"))
...
length(list.files(output_dir, pattern = "_wgs"))
lapply(outfiles, gdalsrsinfo)
# file.remove(infiles)


## >> Step two_Clip layes extent to remove Antarctica ####
new_extent <- "-180 -60 180 90"

## IF step one is not run...
# infiles <- cov_files ## if step one is not run
...
new_res <- res(globalmask)
new_crs = crs(globalmask)@projargs # new_crs = '+proj=eqearth +ellips=WGS84 +wktext'
...
length(list.files(output_dir, pattern = "_reproj"))
lapply(outfiles, gdalsrsinfo)
gdalinfo(outfiles[1])
raster(outfiles[1])


## >> Step four_Clip s.t. #cells in layer equal #cells in mask ####
new_extent <- extent(raster(globalmask))
...
list.files(output_dir, pattern = "_reproj.tif")
file.remove(list.files(output_dir, pattern = "_reproj.tif", full.names = TRUE))
list.files(output_dir, pattern = "_clip2.tif")
file.remove(list.files(output_dir, pattern = "_clip2.tif", full.names = TRUE))
list.files(output_dir, pattern = "_nodataval.tif")
file.remove(list.files(output_dir, pattern = "_nodataval.tif", full.names = TRUE))
...
biofuture <- list.files(file.path(gsdms_dir, "CHELSA/future/mean"),
                        full.names = TRUE, recursive = TRUE)
...
## EXTRAS ####
# ## ------------------------------------------------------------------------- ##
# ## Prepare CHELSA climate layers - PARK THIS ####
# ## ------------------------------------------------------------------------- ## 
# ## >> Current
# biocurrent <- list.files(file.path(gsdms_dir, "CHELSA/envicloud/chelsa/chelsa_V2/GLOBAL/climatologies/1981-2010/bio"),
#                          full.names = TRUE, recursive = TRUE)
