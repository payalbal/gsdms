...
covariates <- readRDS(paste0(data_gsdms, "/covariates_all.rds"))
...
plot(covariates[["bulkdens"]], col = viridis(100))
  ...
  nona <- which(!is.na(values(global_mask)))
    ...
    ## Compare indices (nas) across all soil variables 
  ...
  ## Index of NA values in soil variable where global_mask is non-NA
  ...
  ## Check if NA values have been removed from global_mask
  nona <- which(!is.na(values(global_mask)))
  nas <- which(is.na(values(covariates[["bulkdens"]])[nona]))
  xys <- xyFromCell(global_mask, nona)
  xys <- xys[nas,]
...
plot(covariates[["slope"]], col = viridis(100))
  ...
  nona <- which(!is.na(values(global_mask)))
    ...
    ## Compare indices (nas) across soil variables 
    ##    - indicates these are same across soil variables
...
writeRaster(global_mask, filename = paste0(data_gsdms, "/globalmask10m_treated.tif"))  
global_mask2 <- raster(paste0(data_gsdms, "/globalmask10m_treated.tif"))
  
## gdal_mask on all cvovariates
file_out <- list.files(paste0(data_gsdms, "/covariates"), full.names = TRUE)
file_out2 <- sub("_treated", "_newmask", file_out)
mapply(gdal_mask, inpath = file_out, outpath = file_out2,
       MoreArgs = list(mask=paste0(data_gsdms, "/globalmask10m_treated.tif")))

file_out <- list.files(paste0(data_gsdms, "/covariates"), full.names = TRUE)
covariates2 <- stack(file_out2)
